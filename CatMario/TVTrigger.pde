// Points in the level that trigger TV enemies
class TVTrigger extends Trigger {
  ...
  void run(LevelLayer layer, Actor actor, float[] intersection) {
    ...
    // and remove this trigger so that it's not repeated. Unless we want infinite TVs!
    removeTrigger();
  }
}


// Points in the level that trigger TV nemies
class TVTrigger2 extends Trigger {
  // these values will let us position a new TV when triggered 
  float TV_x, TV_y;
